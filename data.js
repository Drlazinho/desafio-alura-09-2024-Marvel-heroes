let data = [
        {
            "nome": "Wanda Maximoff",
            "alias": "Feiticeira Escarlate",
            "afiliacoes": [
                "Vingadores",
                "M-Force",
                "Feiticeiras de Salem"
            ],
            "poderes": [
                "Manipulação da realidade",
                "Telecinese",
                "Energia mágica",
                "Constructos de energia"
            ],
            "fraquezas": [
                "Instabilidade emocional",
                "Dependência de seus poderes"
            ],
            "primeiraAparicao": "X-Men #4",
            "biografia": "...",
            "equipamentos": [
                "Varinha"
            ],
            "relacionamentos": {
                "familiares": [
                    "Magneto (pai adotivo)",
                    "Pietro Maximoff (Mercúrio)"
                ],
                "românticos": [
                    "Visão"
                ],
                "amigos": [
                    "Vingadores"
                ],
                "inimigos": [
                    "Magneto",
                    "Doctor Doom"
                ]
            },
            "universo": "Marvel-616",
            "bg": "https://wallpapers.com/images/hd/wanda-maximoff-scarlet-witch-evt29k972vz3ewq5.jpg"
        },
        {
            "nome": "Peter Parker",
            "alias": "Spider-Man",
            "afiliacoes": [
                "Vingadores",
                "Defensores"
            ],
            "poderes": [
                "Super força",
                "Agilidade",
                "Sentido aranha",
                "Adesão a superfícies"
            ],
            "fraquezas": [
                "Radioatividade",
                "Instabilidade emocional"
            ],
            "primeiraAparicao": "Amazing Fantasy #15",
            "biografia": "Um adolescente mordido por uma aranha radioativa, Peter Parker adquiriu habilidades sobre-humanas e se tornou o super-herói conhecido como Spider-Man.",
            "equipamentos": [
                "Traje de Aranha",
                "Lançadores de teia"
            ],
            "relacionamentos": {
                "familiares": [
                    "May Parker (tia)",
                    "Ben Parker (tio falecido)"
                ],
                "românticos": [
                    "Mary Jane Watson",
                    "Gwen Stacy"
                ],
                "amigos": [
                    "Harry Osborn",
                    "Ned Leeds"
                ],
                "inimigos": [
                    "Dr. Octopus",
                    "Green Goblin",
                    "Venom"
                ]
            },
            "universo": "Marvel-616",
            "bg": "./assets/spider-man.jpg"
        },
        {
            "nome": "Tony Stark",
            "alias": "Homem de Ferro",
            "afiliacoes": [
                "Vingadores",
                "Stark Industries"
            ],
            "poderes": [
                "Inteligência genial",
                "Mestre em engenharia"
            ],
            "fraquezas": [
                "Dependência da armadura",
                "Ego"
            ],
            "primeiraAparicao": "Tales of Suspense #39",
            "biografia": "Bilionário e engenheiro brilhante, Tony Stark foi ferido em uma expedição e forçado a construir uma armadura para sobreviver. Tornou-se o Homem de Ferro, um super-herói dedicado a proteger o mundo.",
            "equipamentos": [
                "Armadura Mark I, Mark II, Mark III, ...",
                "Repulsores",
                "Inteligência Artificial (JARVIS)"
            ],
            "relacionamentos": {
                "amigos": [
                    "Steve Rogers",
                    "Bruce Banner",
                    "Pepper Potts"
                ],
                "inimigos": [
                    "Mandarim",
                    "Obadiah Stane"
                ]
            },
            "universo": "Marvel-616",
            "bg": "https://postergami.com/wp-content/uploads/2022/01/poster-wallpaper-homem-de-ferro-25.jpg"
        },
        {
            "nome": "Thor Odinson",
            "alias": "Thor",
            "afiliacoes": [
                "Vingadores",
                "Asgardianos"
            ],
            "poderes": [
                "Super força",
                "Durabilidade",
                "Voo",
                "Manipulação do clima",
                "Mjolnir"
            ],
            "fraquezas": ["Encantamento de Odin"],
            "primeiraAparicao": "Journey into Mystery #83",
            "biografia": "Príncipe de Asgard e filho de Odin, Thor é um deus do trovão com poderes cósmicos. Após se provar digno, ele recebeu o martelo Mjolnir e se tornou um defensor da Terra.",
            "equipamentos": [
                "Mjolnir",
                "Armadura Asgardiana"
            ],
            "relacionamentos": {
                "familiares": [
                    "Odin (pai)",
                    "Loki (irmão)",
                    "Frigga (mãe)"
                ],
                "amigos": [
                    "Vingadores",
                    "Sif"
                ],
                "inimigos": [
                    "Loki",
                    "Hela",
                    "Surtur"
                ]
            },
            "universo": "Marvel-616",
            "raça": "Asgardiano",
            "bg": "https://maelstrom-earth.wdfiles.com/local--files/char:thor-odinson/thor-odinson.jpg"
        },
        {
            "nome": "Steve Rogers",
            "alias": "Capitão América",
            "afiliacoes": [
                "Vingadores",
                "S.H.I.E.L.D."
            ],
            "poderes": [
                "Super força",
                "Agilidade",
                "Durabilidade",
                "Liderança"
            ],
            "fraquezas": [
                "Idade avançada",
                "Dependência do soro do supersoldado"
            ],
            "primeiraAparicao": "Captain America Comics #1",
            "biografia": "Um jovem frágil, Steve Rogers foi transformado em um super-soldado durante a Segunda Guerra Mundial. Após décadas congelado, ele despertou no presente para se tornar um dos maiores heróis da Terra.",
            "equipamentos": [
                "Escudo de vibranium"
            ],
            "relacionamentos": {
                "amigos": [
                    "Bucky Barnes",
                    "Tony Stark",
                    "Natasha Romanoff"
                ],
                "românticos": [
                    "Peggy Carter"
                ],
                "inimigos": [
                    "Red Skull",
                    "Hydra"
                ]
            },
            "universo": "Marvel-616",
            "origem": "Experimento do Soro do Supersoldado",
            "bg": "https://wallpaper4k.top/wp-content/uploads/2023/09/capitao-america-wallpaper-4k-3241-scaled.jpg"
        },
        {
            "nome": "Bruce Banner",
            "alias": "Hulk",
            "afiliacoes": [
                "Vingadores",
                "Líderes"
            ],
            "poderes": [
                "Super força",
                "Durabilidade",
                "Regeneração",
                "Saltos incríveis"
            ],
            "fraquezas": [
                "Raiva",
                "Inteligência diminuída quando transformado"
            ],
            "primeiraAparicao": "The Incredible Hulk #1",
            "biografia": "Cientista brilhante, Bruce Banner foi exposto a radiação gama em um experimento, transformando-o em um monstro verde de força inigualável quando enfurecido.",
            "equipamentos": [],
            "relacionamentos": {
                "amigos": [
                    "Rick Jones",
                    "Betty Ross"
                ],
                "inimigos": [
                    "Abominável",
                    "Líder"
                ],
                "familiares": [
                    "Betty Ross"
                ]
            },
            "universo": "Marvel-616",
            "origem": "Exposição à radiação gama",
            "bg": "https://images4.alphacoders.com/588/588373.jpg"
        },
        {
            "nome": "Natasha Romanoff",
            "alias": "Viúva Negra",
            "afiliacoes": [
                "Vingadores",
                "S.H.I.E.L.D.,",
                "Os Defensores"
            ],
            "poderes": [
                "Mestre em artes marciais",
                "Acrobata",
                "Especialista em espionagem"
            ],
            "fraquezas": [
                "Passado traumático",
                "Dependência do soro da super-soldado (algumas versões)"
            ],
            "primeiraAparicao": "Tales of Suspense #52",
            "biografia": "Treinada desde a infância como assassina, Natasha Romanoff escapou da antiga União Soviética e se tornou uma agente da S.H.I.E.L.D. Conhecida por sua agilidade, inteligência e habilidades de combate, a Viúva Negra é uma das mais habilidosas agentes da Marvel.",
            "equipamentos": [
                "Manoplas elétricas",
                "Bastão",
                "Várias armas"
            ],
            "relacionamentos": {
                "amigos": [
                    "Clint Barton",
                    "Steve Rogers",
                    "Bruce Banner"
                ],
                "românticos": [
                    "Clint Barton",
                    "Bruce Banner"
                ],
                "inimigos": [
                    "General Dreykov",
                    "Treinadores Vermelhos"
                ]
            },
            "universo": "Marvel-616",
            "origem": "Programa de treinamento da Viúva Negra",
            "bg": "https://wallpapers.com/images/featured/viuva-negra-4k-696vbesvgu5c2hsh.jpg"
        },
        {
            "nome": "Clint Barton",
            "alias": "Gavião Arqueiro",
            "afiliacoes": [
                "Vingadores",
                "S.H.I.E.L.D."
            ],
            "poderes": [
                "Mestre em arco e flecha",
                "Acrobata",
                "Especialista em combate corpo a corpo"
            ],
            "fraquezas": [
                "Dependência de suas armas"
            ],
            "primeiraAparicao": "Tales of Suspense #57",
            "biografia": "Ex-artista de circo, Clint Barton se tornou um dos melhores arqueiros do mundo. Após treinar com o Homem de Ferro, ele se tornou o Gavião Arqueiro, um dos fundadores dos Vingadores.",
            "equipamentos": [
                "Arco composto",
                "Flechas explosivas, de impacto, etc."
            ],
            "relacionamentos": {
                "amigos": [
                    "Hawkeye",
                    "Black Widow",
                    "Kate Bishop"
                ],
                "inimigos": [
                    "Ronin",
                    "Pistoleiro"
                ]
            },
            "universo": "Marvel-616",
            "origem": "Treinamento autodidata e com o Homem de Ferro",
            "bg": "https://poltronanerd.com.br/wp-content/uploads/2017/08/gavi%C3%A3o.jpg"
        },
        {
            "nome": "Maria Hill",
            "alias": "Agente 13",
            "afiliacoes": ["S.H.I.E.L.D., Vingadores"],
            "poderes": ["Mestre em artes marciais", "Especialista em espionagem"],
            "fraquezas": ["Dependência de tecnologia"],
            "primeiraAparicao": "Daredevil #261",
            "biografia": "Uma agente de campo da S.H.I.E.L.D. com habilidades excepcionais em combate e liderança, Maria Hill é uma das figuras mais importantes da organização.",
            "equipamentos": "Várias armas, Dispositivos tecnológicos",
            "relacionamentos": {
                "amigos": ["Nick Fury",
                "Steve Rogers",
                "Natasha Romanoff"],
                "inimigos": "Hydra"
            },
            "universo": "Marvel-616",
            "origem": "Programa de treinamento da S.H.I.E.L.D.",
             "bg": "https://wallpapers.com/images/hd/maria-hill-1920-x-1080-wallpaper-az7loa2pxroilsz9.jpg"
        },
        // {
        //     "nome": "Nicholas Joseph Fury",
        //     "alias": "Nick Fury",
        //     "afiliacoes": "S.H.I.E.L.D.",
        //     "poderes": "Liderança, Estratégias militares, Experiência em combate",
        //     "fraquezas": "Idade, Perda de um olho",
        //     "primeiraAparicao": "Sgt. Fury and his Howling Commandos #1",
        //     "biografia": "Veterano da Segunda Guerra Mundial, fundou e liderou a S.H.I.E.L.D., uma organização dedicada à proteção global. É conhecido por sua inteligência, experiência e lealdade.",
        //     "equipamentos": "Olho biônico, Armas de fogo",
        //     "relacionamentos": {
        //         "amigos": [
        //             "Steve Rogers",
        //             "Maria Hill"
        //         ],
        //         "inimigos": "Hydra, Red Skull"
        //     },
        //     "universo": "Marvel-616",
        //     "origem": "Soldado da Segunda Guerra Mundial"
        // }
    ]
